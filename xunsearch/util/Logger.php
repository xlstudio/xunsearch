...
// check arguments
XSUtil::parseOpt(['p', 'c', 'project', 'charset', 'hot', 'del', 'put', 'query', 'import', 'limit']);
...
// long options
$params = ['import', 'del', 'put', 'flush', 'hot', 'clean', 'limit'];
foreach ($params as $_) {
    $k = strtr($_, '-', '_');
...
try {
    ...
    if ($hot !== null) {
        ...
        $result = $log_ready ? $search->setFuzzy()->setLimit($limit)->search($query) : [];
        if (count($result) === 0) {
        ...
        if ($del !== null) {
            ...
            foreach (explode(',', $del) as $word) {
                ...
                $docs = $log_ready ? $search->setFuzzy()->setLimit($limit)->search($word) : [];
                if (count($docs) === 0) {
                    echo '并且没有相关的搜索词。';
                } else {
                    echo '相关词：';
